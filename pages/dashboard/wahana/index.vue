<template>
  <div class="container-product" :style="{ padding: '24px', background: '#fff', minHeight: '360px', margin: '16px 0' }">
    ...
    <a-modal
      class="modal-style"
      :mask-closable="false"
      :title="dataEdit ? 'Update Wahana' : 'Tambah Wahana'"
      :visible="visible"
      :footer="null"
      @cancel="onCancel"
    >
      <LazyFormWahana :data-edit="dataEdit" @onEdited="refreshTable" @onCreated="refreshTable" />
    </a-modal>
  </div>
</template>
...
export default Vue.extend({
  ...
  async asyncData ({ $strapi, $getWahanaWithAuth, redirect, $cookies }) {
    try {
      const getWahanas = $strapi.$wahanas.find($getWahanaWithAuth({ _start: 0, _limit: 10 }))
      const countWahanas = $strapi.$wahanas.count($getWahanaWithAuth())
      const wrapRequest = [getWahanas, countWahanas]
      const res = await Promise.all(wrapRequest)
      const wahanas = res[0]
      const total = res[1]
      return { wahanas, total, start: 0, limit: 10 }
    } catch (err) {
      const { response = {} } = err || {}
  ...
  methods: {
    ...
    async onDeleted () {
      try {
        this.$refs.tableWahana.setLoading(true)
        const res = await this.$strapi.$wahanas.find(this.$getWahanaWithAuth())
        this.wahanas = res
      } catch (err) {
        this.$message.error('Maaf gagal perbaharui wahana, silahkan refresh page')
      } finally {
        this.$refs.tableWahana.setLoading(false)
      }
    },
    onEdit (row) {
      this.dataEdit = row
    ...
    async doSearch ({ start = 0 }) {
      ...
      try {
        this.$refs.tableWahana.setLoading(true)
        const paramsWahanas = this.$getWahanaWithAuth({ name_contains: this.search, _start: start, _limit: 10 })
        const getWahanas = this.$strapi.$wahanas.find(paramsWahanas)
        const countWahanas = this.$strapi.$wahanas.count(this.$getWahanaWithAuth({ name_contains: this.search }))
        const wrapRequest = [getWahanas, countWahanas]
        const res = await Promise.all(wrapRequest)
        wahanas = res[0]
        ...
        this.$message.error('Maaf gagal perbaharui wahanas, silahkan refresh page')
      } finally {
        setTimeout(() => {
          this.wahanas = wahanas
          this.total = totalWahanas
          this.$refs.tableWahana.setLoading(false)
        }, 300)
      }
    }
