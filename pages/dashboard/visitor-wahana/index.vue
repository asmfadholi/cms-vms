  ...
  methods: {
    ...
    async doSearch ({ start = 0 }) {
      ...
      try {
        ...
        const getVistors = this.$strapi['$wahana-visitors'].find(paramsVisitors)
        const countVisitors = this.$strapi['$wahana-visitors'].count(this.$getAuthWithArea({ name_contains: this.search }))
        ...
        this.$message.error('Maaf gagal perbaharui visitor wahana, silahkan refresh page')
      } finally {
        setTimeout(() => {
