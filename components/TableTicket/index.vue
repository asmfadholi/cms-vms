    ...
    >
      <a-space size="middle">
        ...
        <a-popconfirm placement="topLeft" ok-text="Yes" cancel-text="No" @confirm="confirm(row)">
          <template slot="title">
            <p>Apa kamu yakin ingin delete tiket ini?</p>
          </template>
...
export default {
  ...
  methods: {
    ...
    async doDeleteTicket (val) {
      this.loading = true
      try {
        await this.$strapi.$tickets.delete(val.id)
        this.$message.success('Berhasil menghapus tiket')
        await this.$emit('onDeleted')
      } catch (err) {
        this.$message.error('Maaf gagal menghapus tiket, silahkan coba lagi')
        setTimeout(() => {
          this.loading = false
        }, 300)
      }
