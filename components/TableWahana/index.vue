    ...
    >
      <a-space size="middle">
        ...
        <a-popconfirm placement="topLeft" ok-text="Yes" cancel-text="No" @confirm="confirm(row)">
          <template slot="title">
            <p>Apa kamu yakin ingin delete wahana ini?</p>
          </template>
...
export default {
  ...
  methods: {
    ...
    async doDeleteProduct (val) {
      this.loading = true
      try {
        await this.$strapi.$wahanas.delete(val.id)
        // await this.$store.dispatch('indexdb/deleteProduct', val.id)
        this.$message.success('Berhasil menghapus wahana')
        await this.$emit('onDeleted')
      } catch (err) {
        this.$message.error('Maaf gagal menghapus wahana, silahkan coba lagi')
        setTimeout(() => {
          this.loading = false
        }, 300)
      }
