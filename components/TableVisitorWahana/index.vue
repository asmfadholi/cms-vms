  ...
  >
    ...
    >
      ...
      <a-popconfirm placement="topLeft" ok-text="Yes" cancel-text="No" @confirm="confirm(row)">
        <template slot="title">
          <p>Apa kamu yakin ingin delete visitor wahana ini?</p>
        </template>
...
export default {
  ...
  methods: {
    ...
    async doDeleteVisitor (val) {
      this.loading = true
      try {
        await this.$strapi['$wahana-visitors'].delete(val.id)
        this.$message.success('Berhasil menghapus visitor wahana')
        await this.$emit('onDeleted')
      } catch (err) {
        this.$message.error('Maaf gagal menghapus visitor wahana, silahkan coba lagi')
        setTimeout(() => {
          this.loading = false
        }, 300)
      }
